...
function createGridSquares() {
  ...
  for (let i = 0; i < numOfSquares; i++) {
    ...
    gridSquare.addEventListener(
      "mousedown",
      (
        e //при нажатии мыши: - вызывается функция setSquareBackgroundColor(e) для изменения фона квадрата
      ) => setSquareBackgroundColor(e)
    );
    gridSquare.addEventListener(
      "mouseover",
      (
        e //наведении на квадрат  - вызывается функция setSquareBackgroundColor(e) для изменения фона квадрата
      ) => setSquareBackgroundColor(e)
    );
    ...
    gridSquare.addEventListener("dragstart", (e) => {
      //предотвратить перетаскивание квадрата.
      e.preventDefault();
    });
...
penColorPicker.addEventListener("input", (e) => {
  //Когда пользователь выбирает новый цвет,
